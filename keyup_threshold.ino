...
int sensitivities[4] = {999, 999, 999, 999};
...
void loop() 
{
  ...
  if(serialLength) {
    ...
    float topPressure = readPressure(A0);

    bool topTriggered = (topPressure > (triggers[0] ? sensitivities[0] - kutv : sensitivities[0])) ? true : false;

    float rightPressure = readPressure(A1);

    bool rightTriggered = (rightPressure > (triggers[1] ? sensitivities[1] - kutv : sensitivities[1])) ? true : false;
 
    float downPressure = readPressure(A2);
    bool downTriggered = (downPressure > (triggers[2] ? sensitivities[2] - kutv : sensitivities[2])) ? true : false;

    float leftPressure = readPressure(A3);
    bool leftTriggered = (leftPressure > (triggers[3] ? sensitivities[3] - kutv : sensitivities[3])) ? true : false;

    if (debug) {
      Serial.print("Pressure: ");
      Serial.print("top: "    +  String(topPressure) + (topTriggered ? "(X) " : " "));
      Serial.print("right: "   +  String(rightPressure) + (rightTriggered ? "(X) " : " "));
      Serial.print("down: "   +  String(downPressure) + (downTriggered ? "(X) " : " "));
...
void printSens() {
  ...
  Serial.println("top: " + String(sensitivities[0]));
  Serial.println("right: " + String(sensitivities[1]));
  Serial.println("bottom: " + String(sensitivities[2]));
  Serial.println("left: " + String(sensitivities[3]));
  Serial.println();
}
